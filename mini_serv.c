...
void broad_cast(t_data data, char *msg, int exept) {
  ...
  for (int i = 0; i < data.maxc; i ++) {
    ...
    if (FD_ISSET(client->sock_fd, &clients)) {
      ...
      int ret = send(client->sock_fd, msg, strlen(msg), 0);
      if (ret <= 0) {
        // printf("can\'t send\n");
        write(2, "Fatal error send\n", 13);
      }
...
char *generate_message(char *buffer, t_client *client) {
  char *msg = NULL;
  char *messages = NULL;
  int isMsg = 1;
  while (isMsg) {
    ...
    if (isMsg == -1) {
      ...
      sprintf(g_msg, "client %d: %s", client->id, str_join(client->buff, msg));
      client->buff = NULL;
      free(msg);
      messages = str_join(messages, g_msg);
    }
  }
  client->buff = str_join(client->buff, buffer);
  return messages;
}
    ...
    for (int i = 0; i < data.maxc; i ++) {
      ...
      if (FD_ISSET(client->sock_fd, &read_set)) {
        ...
        if (ret <= 0) {
          ...
          buffer[ret] = 0x00;
          buffer = generate_message(buffer, client);
